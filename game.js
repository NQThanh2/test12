...
function updateTurnUI() {
    ...
    const playerName = currentPlayer === 0 ? 'Người Chơi 1' :
...
function endGame() {
    ...
    if (scores[0] > scores[1]) {
        icon.textContent = '🏆';
        const winner = 'Người Chơi 1';
        title.textContent = `${winner} Thắng!`;
        msg.textContent = `Chúc mừng! ${winner} đã giành chiến thắng!`;
    } else if (scores[1] > scores[0]) {
        icon.textContent = '🏆';
        const winner = gameMode === 'ai' ? 'Máy' : 'Người Chơi 2';
        title.textContent = `${winner} Thắng!`;
        msg.textContent = gameMode === 'ai'
            ? 'Máy tính đã thắng. Hãy thử lại!'
